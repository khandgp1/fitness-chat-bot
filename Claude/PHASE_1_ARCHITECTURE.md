...
| D15 | Design-plane write model | Direct write, git-versioned; no second approval queue. **Revised 2026-07-08 (operator, Stage 7):** narratives are no longer git-versioned — audit events + daily pre-image snapshots replace git for narratives; prompts keep git | Operator decision (2026-07-07): operator is present live; narratives are derivative of SQLite history, so git bought nothing the system reads |
| D16 | Persistence (knowledge) | Prompts/few-shots/narratives as markdown files; prompts in repo, narratives in a separate private git directory. **Revised 2026-07-08 (operator, Stage 7):** narratives live at `v2/data/narratives/` (gitignored via `data/`, no nested git), daily pre-image snapshots under `history/` | Operator decision (2026-07-07, revised 2026-07-08); simplicity chosen over physical separation, trade-off discussed and recorded |
